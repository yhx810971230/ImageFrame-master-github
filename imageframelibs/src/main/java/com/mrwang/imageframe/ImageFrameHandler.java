...
public class ImageFrameHandler implements WorkHandler.WorkMessageProxy {
     ...
     */
    @Deprecated
    public void loadImage(final String fileDir, int width, int height, int fps, OnImageLoadListener onPlayFinish) {
        if (!isRunning) {
            isRunning = true;
            this.width = width;
            this.height = height;
     ...
     */
    @Deprecated
    public void loadImage(final File[] files, int width, int height, int fps, OnImageLoadListener onPlayFinish) {
        if (!isRunning) {
    ...
    private void setImageFrame(final File[] files, int width, int height, int fps, OnImageLoadListener onPlayFinish) {
        this.width = width;
        this.height = height;
        if (imageCache == null) {
            imageCache = new ImageCache();
    ...
    private void setImageFrame(Resources resources, @RawRes int[] resArray, int width, int height, int fps,
                               OnImageLoadListener onPlayFinish) {
        this.width = width;
        this.height = height;
...
//        resources = null;
    ...
    private void loadInThreadFromFile(final File[] files) {
        if (index < files.length) {
            ...
            if (file.isFile() && isPicture(file)) {
                ...
                bitmapDrawable = BitmapLoadUtils.decodeSampledBitmapFromFile(file.getAbsolutePath(), width, height,
    ...
    private void loadInThreadFromRes(final int[] resIds) {
        if (index < resIds.length) {
            if (null != resources) {
                ...
                long start = System.currentTimeMillis();
                bitmapDrawable = BitmapLoadUtils.decodeSampledBitmapFromRes(resources, resId, width, height,
                        imageCache, isOpenCache);
                long end = System.currentTimeMillis();
                float updateTime = (frameTime - (end - start)) > 0 ? (frameTime - (end - start)) : 0;
                Message message = Message.obtain();
    ...
    public static class ResourceHandlerBuilder implements FrameBuild {
        ...
        public ImageFrameHandler build() {
            if (!imageFrameHandler.isRunning) {
                clip();
                imageFrameHandler.setImageFrame(resources, resArray, width, height, fps, onPlayFinish);
            }
            return imageFrameHandler;
        }
